            ...
            if sit.size and sit:  ###
                ...
                if incone[loop1 + 1] == 0:  # incone[loop1] != 0 and incone[loop1+1] == 0:  # 可以确定在这个时间范围内被看到
                    ...
                    if t_root.size and t_cos - tcp[incone[loop1] - 1] <= t_root[0] <= t_cos - tcp[
                        ...
                        tcp1 = z2t(bb)
                        # 20230221
                        delta_t = np.array(((t_cos - t_root[0]) - tcp0) / (tcp1 - tcp0)).squeeze()  # both negative values
                        ...
                        if in_cone_snap(findout, incone[loop1]):
                            ...
                            for i, iprop in enumerate(cb.props):
                                if iprop == cb.pos_name or iprop in cb.pos_name or iprop == cb.vel_name or iprop in cb.vel_name:
                                    ...
                                    exec("%s = interpo(ga[id2].%s[ssnow - st - 1], ga[id2].%s[ssnow - st], delta_t)" % (iprop, iprop, iprop))
                                else:
                            ...
                            if len(cb.pos_name) == 1:
                                ...
                                for loop2 in range(3):
                                    exec("VELOCITOES[loop2] = interpo(ga[id2].%s[ssnow - st - 1][0], ga[id2].%s[ssnow - st][0], delta_t)" % (cb.vel_name[0], cb.vel_name[0]))
                                exec("%s = VELOCITOES" % cb.vel_name[0])
                            else:
                                for ii in range(3):
                                    exec("%s = findout[ii]" % cb.pos_name[ii])
                                    exec(
                                        "VELOCITOES[loop2] = interpo(ga[id2].%s[ssnow - st - 1][0], ga[id2].%s[ssnow - st][0], delta_t)" % (
                                        cb.vel_name[ii], cb.vel_name[ii]))

                            v_los = (findout[0] * VELOCITOES[0] + findout[1] * VELOCITOES[1] + findout[2] * VELOCITOES[2]) / d_comoving
