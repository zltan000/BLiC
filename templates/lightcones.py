# 20230428: 加入不插值版本光锥（直接拼接）: np.sum(interpo_method) == 0
# 20230512: change some 'exec' lines to getattr() function
    ...
    for loop13 in range(a, b - 1, -1):  # 红移由低到高
        ...
        if snap_index.size:
            if len(cb.pos_name) == 1:
                p0 = getattr(ga[id2], cb.pos_name[0])[snap_index] * cb.pos_unit
            else:
                p0 = np.zeros(3)
                for ii in range(3):
                    p0[ii] = getattr(ga[id2], cb.pos_name[ii])[snap_index] * cb.pos_unit
            ...
            if sit.size and sit:  ###
                ...
                if incone[loop1 + 1] == 0:  # incone[loop1] != 0 and incone[loop1+1] == 0:  # 可以确定在这个时间范围内被看到
                    if np.sum(interpo_method):
                        ...
                        if len(cb.pos_name) == 1:
                            p0 = getattr(ga[id2], cb.pos_name[0])[ssnow - st] * cb.pos_unit
                        else:
                            p0 = np.zeros(3)
                            for ii in range(3):
                                p0[ii] = getattr(ga[id2], cb.pos_name[ii])[ssnow - st] * cb.pos_unit
                        p0[0] = (phase[0][0] * boxsize + phase[0][1] * p0[0]) % boxsize + xi * boxsize - obs[0]
                        p0[1] = (phase[1][0] * boxsize + phase[1][1] * p0[1]) % boxsize + yi * boxsize - obs[1]
                        p0[2] = (phase[2][0] * boxsize + phase[2][1] * p0[2]) % boxsize + zi * boxsize - obs[2]
                        func0 = lambda tcos: \
                        ...
                        if in_cone_snap(findout, incone[loop1]):
                            ...
                            if len(cb.pos_name) == 1:
                                ...
                                for loop2 in range(3):
                                    VELOCITOES[loop2] = interpo(getattr(ga[id2], cb.vel_name[0])[ssnow - st - 1][loop2],
                                                                getattr(ga[id2], cb.vel_name[0])[ssnow - st][loop2],
                                                                delta_t)
                                # locals()[cb.vel_name[0]] = VELOCITOES
                                exec("%s = VELOCITOES" % cb.vel_name[0])
                            else:
                                for ii in range(3):
                                    exec("%s = findout[ii]" % cb.pos_name[ii])
                                    VELOCITOES[ii] = interpo(getattr(ga[id2], cb.vel_name[ii])[ssnow - st - 1],
                                                             getattr(ga[id2], cb.vel_name[ii])[ssnow - st], delta_t)
...
class galaxy():
    def __init__(self, trackid):
        trackid = int(trackid)
        start_ssn = k
        dict_gal = {p: [] for p in fields}  # p is name
        ...
        for ii in range(k, ssn):  # ssn 从小到大读、存  0对应刚出生的ssn  # 改为k
            for p in fields:
                dict_gal[p].append(data[ii][trackid][p])
                # exec("%s.append(data[ii][trackid]['%s'])" % (fields[p], fields[p]))
            live.append(ii)
            livetime.append(t_cos - tcp[ii])  # 便于插值，换算为以z=3000时为时间零点计算
        for p in fields:
            setattr(self, p, np.array(dict_gal[p]))  # 2022.7.1新增np.array
            # exec('self.%s = np.array(dict_gal[p])' % p)
        self.snapnum = np.array(live)
        self.time = np.array(livetime)
        # fitting traces
        if len(cb.pos_name) == 1 and len(cb.vel_name) == 1:
            self.coeffX = fit_coeff(self.time, getattr(self, cb.pos_name[0])[:, 0], getattr(self, cb.vel_name[0])[:, 0])
            self.coeffY = fit_coeff(self.time, getattr(self, cb.pos_name[0])[:, 1], getattr(self, cb.vel_name[0])[:, 1])
            self.coeffZ = fit_coeff(self.time, getattr(self, cb.pos_name[0])[:, 2], getattr(self, cb.vel_name[0])[:, 2])
        elif len(cb.pos_name) == 3 and len(cb.vel_name) == 3:
            self.coeffX = fit_coeff(self.time, getattr(self, cb.pos_name[0]), getattr(self, cb.vel_name[0]))
            self.coeffY = fit_coeff(self.time, getattr(self, cb.pos_name[1]), getattr(self, cb.vel_name[1]))
            self.coeffZ = fit_coeff(self.time, getattr(self, cb.pos_name[2]), getattr(self, cb.vel_name[2]))
        elif len(cb.vel_name) == 0:
            if len(cb.pos_name) == 1:
                self.coeffX = linear_fit(self.time, getattr(self, cb.pos_name[0])[:, 0])
                self.coeffY = linear_fit(self.time, getattr(self, cb.pos_name[0])[:, 1])
                self.coeffZ = linear_fit(self.time, getattr(self, cb.pos_name[0])[:, 2])
            elif len(cb.pos_name) == 3:
                self.coeffX = linear_fit(self.time, getattr(self, cb.pos_name[0]))
                self.coeffY = linear_fit(self.time, getattr(self, cb.pos_name[1]))
                self.coeffZ = linear_fit(self.time, getattr(self, cb.pos_name[2]))
        else:
